...
bounds_t get_range_input(std::string type){
  ...
  bound.up = input + 2*range;
  bound.low = input - 2*range;
...
double position, velocitynew = 0, Vel_par, Vel_perp;
pair_t new_pair;
...
//Now set the radii to the same radii as the original pair
new_pair.a.r200b = old_pair.a.r200b;
