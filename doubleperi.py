#counts how many mergers all the pairs in our catalog have gone through and only saves the pairs that have had exactly 1
...
#unpack data
#have to change this based on which data file we are looking at
...
count = 0 #counter that tracks how many single pass mergers we have
...
for i in range(len(massa)):
    ...
    # create arrays of all the separation and velocity data
    ...
    for dist in range(len(initseplist)):
        #zeros in the data arrays are dummy values, so if there is real data, then it saves it in a new data array that doesn't have any dummy values
    ...
    #if the pair has had exactly one pass through, we write out the current data
    if passes == 1:
        count += 1
