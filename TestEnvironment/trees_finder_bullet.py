...
pairid, rockstarida, rockstaridb, massa, massb, radiusa, radiusb, mainleafa, mainleafb, depthfirsta, depthfirstb, separation, vel_on, vel_off = np.loadtxt('bullet_separation_data.csv', delimiter = ',', unpack = True)

#unpack the trees
filearray = [10,20,30,35,40,45]#,50,55,60,65,70] Don't need these in bullet
percentagecounter = 0
f = open("treefile_bullet.csv", "w")
...
for file in filearray:
	...
	for i in range(len(pairid)):
		...
		for k in range(len(trowid)):
			if (tmainleaf[k] == mainleafa[i] and tdepthfirstid[k] < depthfirsta[i]) or (tmainleaf[k] == mainleafb[i] and tdepthfirstid[k] < depthfirstb[i]):
